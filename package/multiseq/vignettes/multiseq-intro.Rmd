...
```
In the Figure above we plotted the underlying signal in red, the simulated data in black circles, and the estimated baseline mean in dark green (with the light green curves reporting the baseline mean +/- 2 times the posterior standard errors- not yet).
...
Notice the use of argument `minobs` in function `multiseq` (type `?multiseq` to see a complete list of possible arguments) which can be used to specify the minimum number of observations ...required to be .. in each logistic model.
...
Function `get.counts` extracts data in *bam*, *bigWig*, or *hdf5* format using `samtools`, the *UCSC tools*, or the **R** package **rhdf5**, respectively, and puts them into a matrix that can be fed as input to `multiseq`. Function `get.counts` requires as input a samplesheet in the following format:
